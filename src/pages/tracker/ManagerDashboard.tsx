...
export default function ManagerDashboard() {
    ...
    return (
      <div className="flex h-[70vh] items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium animate-pulse">Memuat data dashboard...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 pb-12 animate-in fade-in duration-500">
      ...
      <div className="bg-white dark:bg-slate-800/80 backdrop-blur-md p-5 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 sticky top-16 z-30 transition-all hover:shadow-md">
        <div>
          <h1 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-700 to-indigo-600">
            Manager Dashboard
          </h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 font-medium">Ringkasan performa dan aktivitas teknisi</p>
        </div>
        
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
          <div className="flex items-center bg-slate-50 dark:bg-slate-900/50 rounded-xl p-1.5 border border-slate-200 dark:border-slate-700 flex-1 lg:flex-none">
            <CalendarIcon className="w-4 h-4 text-slate-400 dark:text-slate-500 ml-2 mr-1" />
            <select 
              className="bg-transparent text-slate-700 dark:text-slate-300 rounded-lg p-1.5 focus:ring-0 outline-none text-sm font-semibold w-full cursor-pointer"
              value={selectedDate}
          ...
          <div className="flex items-center bg-slate-50 dark:bg-slate-900/50 rounded-xl p-1.5 border border-slate-200 dark:border-slate-700 flex-1 lg:flex-none">
            <MapPin className="w-4 h-4 text-slate-400 dark:text-slate-500 ml-2 mr-1" />
            <select 
              className="bg-transparent text-slate-700 dark:text-slate-300 rounded-lg p-1.5 focus:ring-0 outline-none text-sm font-semibold w-full cursor-pointer"
              value={selectedWitel}
          ...
          <button 
            ...
            className="flex items-center justify-center gap-2 bg-slate-100 dark:bg-slate-700 hover:bg-red-50 text-slate-700 dark:text-slate-300 hover:text-red-600 px-3.5 py-2.5 rounded-xl font-semibold transition-all border border-slate-200 dark:border-slate-700 hover:border-red-200 active:scale-95 w-full lg:w-auto"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 hover:shadow-md transition-all h-[420px] flex flex-col">
        ...
        {selectedWitel === "ALL" && witelData.length > 0 && (
          <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 hover:shadow-md transition-all h-[420px] lg:col-span-2 flex flex-col">
            <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-6 flex items-center gap-2">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-amber-200 flex flex-col h-[500px] overflow-hidden relative">
          ...
          <div className="p-5 overflow-y-auto flex-1 space-y-3 custom-scrollbar">
            {filteredTasks
              ...
              .slice(0, 15)
              .map(t => (
              <div key={t.id} className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-amber-100 shadow-sm hover:shadow-md transition-shadow group relative overflow-hidden cursor-pointer" onClick={() => openModal(`Detail Kendala: ${t.order}`, [t])}>
                ...
                <div className="pl-2">
                  <p className="text-xs text-slate-600 dark:text-slate-300 font-semibold mb-1.5 flex items-center gap-1.5">
                    <Users className="w-3.5 h-3.5 text-slate-400 dark:text-slate-500" /> {t.technicianName}
                  </p>
                  <p className="text-sm text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-900/50 p-2 rounded-lg border border-slate-100 dark:border-slate-700 leading-relaxed italic">
                    "{t.notes}"
                  </p>
                </div>
              </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-blue-200 flex flex-col h-[500px] overflow-hidden relative">
          ...
          <div className="p-5 border-b border-blue-100 bg-blue-50/30">
            ...
            <div className="flex gap-3">
              <div className="flex-1 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 px-3 py-2 flex items-center shadow-sm">
                ...
                <select 
                  className="bg-transparent text-slate-700 dark:text-slate-300 w-full outline-none text-sm font-semibold cursor-pointer"
              ...
              <div className="flex-1 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 px-3 py-2 flex items-center shadow-sm">
                <Activity className="w-4 h-4 text-slate-400 dark:text-slate-500 mr-2" />
                <select 
                  className="bg-transparent text-slate-700 dark:text-slate-300 w-full outline-none text-sm font-semibold cursor-pointer"
          ...
          <div className="p-5 overflow-y-auto flex-1 space-y-3 custom-scrollbar">
            {displayedActiveTasks
              .slice(0, 15)
              .map(t => (
              <div key={t.id} className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-md transition-all group hover:border-blue-200 cursor-pointer" onClick={() => openModal(`Detail Pekerjaan: ${t.order}`, [t])}>
                <div className="flex justify-between items-center mb-2">
                  <span className="font-bold text-slate-800 dark:text-slate-100 text-sm">{t.order}</span>
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm text-slate-800 dark:text-slate-100 font-semibold truncate">{t.technicianName}</p>
      ...
      {/* Detail Data Table */}
      <div className="bg-white dark:bg-slate-800 p-1 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden mt-8">
        <div className="p-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50/50">
      ...
      {modalData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative w-full max-w-6xl bg-white dark:bg-slate-800 rounded-2xl shadow-2xl flex flex-col h-[85vh] overflow-hidden z-10">
            <div className="flex items-center justify-between p-5 border-b border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-t-2xl z-10 shrink-0">
              <div>
              ...
              <button 
                onClick={closeModal} 
                className="p-2 text-slate-400 dark:text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-full transition-colors self-start"
              >
                <X className="w-6 h-6" />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto bg-slate-50 dark:bg-slate-900/50 p-3">
